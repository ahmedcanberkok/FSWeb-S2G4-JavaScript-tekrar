...
function KareninAlani(kenaruzunlugu){
	...
	return  kareAlan = kenaruzunlugu*kenaruzunlugu;
...
/* 	GÖREV 1:  
		- CemberinCevresi fonksiyonunu kullanarak aşağıdaki yönergeleri uygulayın:
			...
			2. Global değişken olarak verilmiş pi sayısı fonksiyon içinde kullanılacak (pi sayısı fonksiyonun parametresi 
...
function CemberinCevresi(yaricap){
	return cembercevre = 2*pi*yaricap ;
}
...
/* 	GÖREV 2:  
		- CemberinAlani fonksiyonunu kullanarak aşağıdaki yönergeleri uygulayın:
			...
			3. Çemberin alanı hesaplanacaktır (💡 İPUCU: Çemberin alanı = pi * yarıçapın karesi, yarıçapın karesini bulmak için 
			Javascript içinde tanımlı Math kütüphanesini kullanabilirsiniz. Math.pow(yaricap,2))
			4. Hesaplanan çemberin alanı döndürülecektir.
		*/
		
function CemberinAlani(yaricapi,pi){
 	const yaricapkaresi = Math.pow(yaricapi,2);
 	const cemberAlani = Math.round(pi);
   	return cemberAlan = pi*yaricapkaresi ;


}

/* (Oto test yok) Yukarıdaki CemberinAlani fonksiyonunu yarıçap = 15 vererek aşağıda çalıştırıp, 
sonucu konsolda gözlemleyin (console.log)  */
...
/* 	GÖREV 3:
		- Sayfanın en üstünde global değişken olarak tanımlanmış bir sayilar dizisi bulunmaktadır. Bu dizi içinde 0 ile 1000 arasında
		 rasgele oluşturulmuş tam sayılar ve ondalıklı sayılar bulunmaktadır. Bu diziyi kullanarak aşağıdakileri uygulayın:
			...
			3b. `ucetambolunenler` adında bir dizi tanımlayın ve bu diziye sayilar dizisindeki
...
diziuz (); 
	...
	function enbenk () {
		...
		for (var i = 0; i < sayilar.length; i++) {
			...
			console.log( "En büyük değer : " , enbüyük);
			console.log("En küçük değer : " ,enkücük);
	}
	enbenk ()
